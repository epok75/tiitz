...
class GeneratorController {
	...
	public function generateEntityAction() {
		...
		require_once ROOT . '/app/components/tzSQL.class.php';

		$tzsql = tzSQL::getInstance('localhost','root','root','ARGOS_WSM');

		$list = $tzsql::$tzPDO->prepare('show tables');
		$list->execute();

		$tables = $list->fetchAll();

		if(empty($_POST['tablename']))
			require_once ROOT.'/app/components/gui/views/formGenerator.php';
		else{
		...
		foreach ($_POST['tablename'] as $tablename) {
			...
			$colums = $tzsql::$tzPDO->prepare($sql);
			$colums->execute();
			...
			class $class extends Entity {
				...
				public function __construct($" . "tzSQL){
					$" . "this->tzSQL = $" . "tzSQL;
				}
				...
				public function Delete(){
					...
					if(!empty($" . "this->id)){
						...
						$" . "result = tzSQL::$" . "tzPDO->prepare($" . "sql);
						$" . "result->execute();
